...
describe('test customer user dao functions:', () => {
    ...
    it('update user', async function () {
        ...
        assert.strictEqual(user[0].role, "role");

    });
    it('update user empty userName', async function () {
        var actual = await customer_dao.update_user(1,"", "password", "email","role");
        assert.strictEqual(actual, -1);

    });
    it('update user short password', async function () {
        var actual = await customer_dao.update_user(1,"user", "123", "email","role");
        assert.strictEqual(actual, -1);

    });
    it('update user empty mail', async function () {
        var actual = await customer_dao.update_user(1,"user", "123", "","role");
        assert.strictEqual(actual, -1);

    });
    it('update user empty password', async function () {
        var actual = await customer_dao.update_user(1,"user", "", "email","role");
        assert.strictEqual(actual, -1);

    });
    it('insert ticket', async function () {
        await connectedKnex.raw(`select * from sp_insert_user('user','pass','mail','role')`);
