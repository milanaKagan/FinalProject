...
describe('test anonymous user dao functions:', () => {
    beforeEach(async function () {
        ...
        await connectedKnex.raw(`select * from sp_insert_user('${user1.username}','${user1.pasword}','${user1.email}','${user1.role}')`);
    ...
    it('insert user', async function () {
        var actual = await anon_dao.insert_user("user", "password", "email","role");
        assert.strictEqual(actual, '2');

    });
    it('insert user empty role', async function () {
        var actual = await anon_dao.insert_user("username", "password", "email","");
        assert.strictEqual(actual, -1);

    });
    it('insert user empty userNamw', async function () {
        var actual = await anon_dao.insert_user("", "password", "email","role");
        assert.strictEqual(actual, -1);

    });
    it('insert user short password', async function () {
        var actual = await anon_dao.insert_user("user", "123", "email","role");
        assert.strictEqual(actual, -1);

    });
    it('insert user empty mail', async function () {
        var actual = await anon_dao.insert_user("user", "123", "","role");
        assert.strictEqual(actual, -1);

    });
    it('insert user empty password', async function () {
        var actual = await anon_dao.insert_user("user", "", "email","role");
        assert.strictEqual(actual, -1);
